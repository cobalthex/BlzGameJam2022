...
std::ostream& operator <<(std::ostream& os, const ResourceDef& def)
{
	return (os << _AnyToJson(def));
}
std::ostream & operator <<(std::ostream & os, const Resource & resource)
{
	return (os << _AnyToJson(resource));
}
std::ostream & operator <<(std::ostream & os, const ProductionScale & scale)
{
	return (os << _AnyToJson(scale));
}
std::ostream & operator <<(std::ostream & os, const ProductionDef & def)
{
	return (os << _AnyToJson(def));
}
std::ostream& operator <<(std::ostream& os, const BuildingDef& def)
{
	return (os << _AnyToJson(def));
}
...
void to_json(nlohmann::json& json, const ProductionDef& def)
{
	json["id"] = static_cast<size_t>(def.id);
	json["name"] = def.name;
	json["discipline"] = static_cast<size_t>(def.discipline);
	json["resourceScalar"] = def.resourceScalar;
	json["timeScalar"] = def.timeScalar;
	json["duration"] = std::to_string(std::chrono::duration_cast<SecondsF>(def.duration).count()) + "s";
...
void to_json(nlohmann::json& json, const BuildingDef& def)
{
	...
	json["name"] = def.name;
	json["zone"] = def.name;
	json["production"] = def.production;
}
